...
var queueInnerHTML = '<p class="system-status-queue">0</p>',
    queueDeluxeHTML = '<p class="system-status-queue" style="color: #0fbf19; background-color: #0fbf19;">0</p>',
    queueNormalHTML = '<p class="system-status-queue" style="color: #14fb01; background-color: #14fb01;">0</p>',
    repairInnerHTML = '<p class="system-status-queue" style="color: #8296a7; background-color: #8296a7;">0</p>';
...
var resultSimTime = document.getElementById("resultSimTime"),
    ...
    resultRepairmanUtilization = document.getElementById("resultRepairmanUtilization"),
...
var allMachines = [],
    ...
    deluxeQueue = [],
    normalQueue = [],
    deluxeRepairman, normalRepairman,
    currentMachine, repairingDeluxe, repairingNormal,
    simulationTime, systemIdleTime, systemRevenue, systemRepairmanUtilization, maxQueueSize;
var fireNextEvent;
...
function startSimulation() {
    ...
    fireNextEvent = setInterval(function() {
        ...
        simulationTime += .25;
        systemRepairmanUtilization = Math.round(((deluxeRepairman.workingHours + normalRepairman.workingHours) / (2 * simulationTime)) * 100).toFixed(2);

        if (repairingDeluxe) {
            deluxeRepairman.workingHours += .25
            repairingDeluxe.repairTime -= .25;
            if (repairingDeluxe.repairTime === 0) {
                repairingDeluxe.breakdownTime = calcBreakdown(0, repairingDeluxe.serviceTime);
        ...
        if (repairingNormal) {
            repairingNormal.repairTime -= .25;
            normalRepairman.workingHours += .25
            if (repairingNormal.repairTime === 0) {
                repairingNormal.breakdownTime = calcBreakdown(0, repairingNormal.serviceTime);
        ...
        if (currentMachine) {
            currentMachine.breakdownTime -= .25;
            if (currentMachine.type === 1) {
                systemRevenue += (.25 * goodUnitsDeluxe.value)
            } else {
                systemRevenue += (.25 * goodUnitsNormal.value)
            }
        } else {
...
function initializeEventList() {
    ...
    var calcDeluxeService = forceDescriptives(randomList(Number(numberDeluxe.value), Number(serviceDeluxe.value) - 5, Number(serviceDeluxe.value) + 5), Number(serviceDeluxe.value), Number(sd.value)),
        calcDeluxeRepair = forceDescriptives(randomList(Number(numberDeluxe.value), Number(repairDeluxe.value) - 2, Number(repairDeluxe.value) + 2), Number(repairDeluxe.value), Number(sd.value)),
        calcDeluxeBreakdown = [];
    for (let i = 0; i < Number(numberDeluxe.value); i++) {
        calcDeluxeBreakdown[i] = calcBreakdown(0, calcDeluxeService[i]);
    }
    var calcNormalService = forceDescriptives(randomList(Number(numberNormal.value), Number(serviceNormal.value) - 5, Number(serviceNormal.value) + 5), Number(serviceNormal.value), Number(sd.value)),
        calcNormalRepair = forceDescriptives(randomList(Number(numberNormal.value), Number(repairNormal.value) - 2, Number(repairNormal.value) + 2), Number(repairNormal.value), Number(sd.value)),
        calcNormalBreakdown = [];
    for (let i = 0; i < Number(numberNormal.value); i++) {
        calcNormalBreakdown[i] = calcBreakdown(0, calcNormalService[i]);
    }
    allMachines = produceMachines(calcDeluxeService, calcDeluxeRepair, calcDeluxeBreakdown, calcNormalService, calcNormalRepair, calcNormalBreakdown);
    ...
    systemIdleTime = 0, systemRevenue = 0, systemRepairmanUtilization = 0, maxQueueSize = 0;
    updateFrontendVariables();
    showResults();
}
...
function showResults() {
    ...
    resultCharges.innerHTML = deluxeRepairman.workingHours * chargesDeluxe.value + normalRepairman.workingHours * chargesNormal.value;
    resultRevenue.innerHTML = systemRevenue;
}

function stopSimulation() {
    clearInterval(fireNextEvent);
    if (numberDeluxe.value < numberNormal.value) maxQueueSize = Math.round(numberNormal.value / 2)
...
function forceDescriptives(list, mean, sd) {
    ...
    var oldDescriptives = descriptives(list),
        ...
        i;
    for (i = 0; i < len; i++) {
        newList[i] = roundToNearest(Math.abs(sd * (list[i] - oldMean) / oldSD + mean));
    }
    return newList;
}